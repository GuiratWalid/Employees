...
<html>
    ...
    <body>
                <%
                        ...
                        ResultSet rs = null;
                        rs = st.executeQuery("select * from employe where login = '"+login+"'");
                        
                        if(rs.next()){
                            String datnais = rs.getString(11);
                %>
                
        <h1>Modifier le profil de l'employé <%=rs.getString(2)%> <%=rs.getString(1)%></h1>
        <br/><br/>
        <form method="post" action="/TP8-war/ModifierProfilEmployes">
            ...
            <input type="text" name="login" id="login" value="<%=rs.getString(7)%>" required/>
